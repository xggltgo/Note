...
**xgglt：rem适配的核心是需要将根元素的字体大小设置为 `设备宽度/设计稿宽度* 100` px**

> xgglt：
>
> REM适配的核心是不同尺寸屏幕拆分为列数，按列数进行布局
>
> 设备宽度 / 设计稿宽度 = 设计稿每列对应的设备宽度（设置为html根字体大小，后续其他元素量出来是多少就能写多少 rem）
>
> 但是，
...
假设设计稿是按照 *750px*（*iPhone 6/7/8* 尺寸）设计的，我们测量出来的是 *100px*，那么换算成设备宽度就是 *50px*。而一列的宽度我们是知道的，因此 *50/3.75* 约等于 *13.33* 列，后面在设置盒子宽度时，就可以设置为 *13.33* * 一列的宽度。
