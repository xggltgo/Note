...
这里我们主要来看 *CSS* 方面能够进行==哪些性能优化==。



1.使用 `id selector` 非常的==高效==。在使用 `id selector` 的时候需要注意一点：因为 ==`id`== 是唯一的，所以不需要既指定 `id` 又指定 `tagName`：
...
3.不要使用 `attribute selector`，如：p[att1=”val1”]。这样的==匹配非常慢==。更不要这样写：`p[id="id1"]`。这样将 `id selector` 退化成 `attribute selector`。
...
4.通常将==浏览器前缀置于前面，将标准样式属性置于最后==，类似：
...
使用 `@import` 引入 `CSS` 会==影响浏览器的并行下载==。使用 `@import` 引用的 `CSS` 文件只有在引用它的那个 `CSS` 文件被下载、解析之后，浏览器才会知道还有另外一个 `CSS` 需要下载，这时才去下载，然后下载后开始解析、构建 `Render Tree` 等一系列操作。
