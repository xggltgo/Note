...
1. 什么是虚拟dom？
   ...
   ```vue
   mounted(){
   	console.log(this._vnode) //查看虚拟dom
   }
   ```
...
3. 虚拟dom是如何转换为真实dom的？
   ...
   在一个组件实例首次被渲染时，它先生成虚拟dom树，然后根据（递归遍历）虚拟dom树创建真实dom，并把真实dom挂载到页面中合适的位置，此时，每个虚拟dom便会对应一个真实的dom。(elm属性对应是真实dom)如果只关注首次渲染的性能的话，我们会发现使用虚拟dom的性能可能比直接创建真实dom的性能更差，因为它比直接创建真实dom多出了创建虚拟dom的过程，然而在更新页面时虚拟dom的作用才真正的体现出来。

   如果一个组件受响应式数据变化的影响，需要重新渲染时，它仍然会重新调用render函数，创建出一个新的虚拟dom树，用新树和旧树对比，通过对比，vue会找到最小更新量，然后更新它们对应的真实dom
...
4. 模板和虚拟dom的关系
   ...
   1. 将模板字符串转换成为`AST`(抽象语法树)（用一种JS的树形结构来描述我们的JS代码）
