...
*CSS3* 新增了==过渡属性==，可以使用从一个状态变化到另一个状态时，变化更加的==平滑==。
...
上面就是一个快速入门示例，接下来我们来看一下 *transition* 的==具体语法==。
...
需要特别说明的是，并不是所有的属性都可以进行过渡，只有属性是==具有一个中点值的属性==才能够进行过渡效果。因为这样才能通过不停的修改中间值从而实现过渡效果。例如 ==*display:none|block*== 就没有中间值，所以无法应用过渡。



能够过渡的属性类型有颜色属性、具有长度值或百分比的属性、阴影、变形系列属性，具体支持过渡的属性可以==参阅下表==：
...
该属性主要用于设置一个属性过渡到另一个属性所需要的==时间==，单位为==秒==(*s*)或者==毫秒==(*ms*)。默认值为 *0*，代表变换是即时的。
...
过渡==延迟==多久触发，单位为==秒==或者==毫秒==。但是值可以是正整数，负整数和 *0*。
...
如果要对多个 *CSS3* 属性应用过渡效果，直接用==逗号分离开==即可，这种主要是针对==每个属性过渡的时间不同的情况==下。
...
有些时候，在 *JS* 中的某些操作需要过渡效果结束后再执行，此时事件类型中给我们提供了一个 ==*transitionend* 事件==方便我们监听过渡效果是否结束，例如：
...
使用 *CSS3* 的 *animation* 制作动画包含==两个部分==：首先是用关键帧声明一个动画，其次是在 *animation* 调用关键帧声明的动画。
...
*animation-iteration-count* 属性定义动画应该==播放多少次==，如果想要一直播放，那么次数就是无限次，所以属性值为 ==*infinite*==。
...
那么 *animation-direction* 就可以设置动画是否要==反向播放==回去。该属性对应的可设置值有：
...
需要==注意==的是，如果动画被设置为只播放一次，该属性将不起作用。
...
*animation-play-state* 属性规定动画正在==运行还是暂停==，语法为：
...
在上面的示例中，我们就通过 *JS* 来控制 *animation-play-state* 从而==切换动画播放与暂停这两个状态==。
...
同样，动画也有对应的事件，与过渡只提供有 *transitionend* 事件不同的是，在 *CSS* 动画播放时，会发生以下==三个事件==：
